...
router.post('/change/ispublicintended/:blogId', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
...
router.get('/public/blogs/:currentUserId', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
...
router.get('/approved/blogs/:currentUserId', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const { currentUserId } = req.params;
...
router.get('/pending/rejected/blog/:currentUserId', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const { currentUserId } = req.params;
...
router.get('/private/blogs/:currentUserId', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const { currentUserId } = req.params;
