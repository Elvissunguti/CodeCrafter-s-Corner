...
router.post('/create/:blogId', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
...
router.post('/edit/:commentId', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const userId = req.user._id;
...
router.post('/parentcomment/:blogId/:parentCommentId', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const userId = req.user._id;
    const { blogId } = req.params;
...
router.get('/fetch/:blogId', async (req, res) => {
  try {
    const { blogId } = req.params;

    // Find all top-level comments (comments without parentCommentId)
    const topLevelComments = await Comment.find({ blogId, parentCommentId: null }).populate({
    ...
    }).populate('userId', 'userName').sort({ createdAt: 'asc' }); // Sort comments by creation time in ascending order
